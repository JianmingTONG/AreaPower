...
funcs = ["fifo","adder_tree","crossbar"]
...
with open("Activation_Function_report.rpt", "w") as f:
  for func in funcs:
    ...
    for i in range(len(test_widths)):
      root_directory = "./result/" + func + "/report" + str(test_widths[i]) 
    ...
    #   power = power_lines[41][65:75]
    #   power_total.append(power)

      with open(timing_path,"r") as timing_report:
        for line in timing_report:
          if "slack" in line and "tt1v25c" not in line and "c_fifo_" not in line and "Design" not in line:
            slack = line[66:-1]
